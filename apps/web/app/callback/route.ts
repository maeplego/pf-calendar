...
export async function GET(req: NextRequest) {
  ...
  if (err) {
    return NextResponse.redirect(new URL(`/host?error=${encodeURIComponent(err)}`, origin));
  }
  ...
  if (!code || !state || !expected || state !== expected || !nonce || !verifier) {
    return NextResponse.redirect(new URL("/host?error=state", origin));
  }
  ...
  if (!tokenRes.ok) {
    return NextResponse.redirect(new URL("/host?error=token", origin));
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/host?error=id_token", origin));
  }
}
